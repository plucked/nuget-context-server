...
public interface IPackageMetadataService
{
    ...
    ///     If no version is specified, details for the latest version (stable or pre-release based on configuration/defaults)
    ///     might be returned,
    ...
    /// <param name="version">
    ///     The specific package version to retrieve details for. If null, the behavior is
    ///     implementation-dependent (e.g., latest version).
    /// </param>
    ...
    /// </returns>
    Task<PackageDetailInfo?> GetPackageDetailsAsync(string packageId, string? version = null,
