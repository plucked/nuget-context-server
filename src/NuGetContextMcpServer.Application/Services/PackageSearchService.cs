...
public class PackageSearchService : IPackageSearchService
{
        ...
        int skip, int take, CancellationToken cancellationToken)
    {
        _logger.LogInformation(
            "Searching packages for term: {SearchTerm}, IncludePrerelease: {IncludePrerelease}, Skip: {Skip}, Take: {Take}",
            searchTerm, includePrerelease, skip, take);
        try
        {
            var pagedResults =
                await _nugetQueryService.SearchPackagesAsync(searchTerm, includePrerelease, skip, take,
                    cancellationToken);
            var packageSearchResults = pagedResults as PackageSearchResult[] ?? pagedResults.ToArray();
            var resultCount = packageSearchResults.Count();
