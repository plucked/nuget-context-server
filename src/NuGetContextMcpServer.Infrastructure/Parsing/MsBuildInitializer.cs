...
// Add logger for better diagnostics
    ...
    public static void EnsureMsBuildRegistered(ILoggerFactory? loggerFactory = null)
    {
        ...
        lock (_lock)
        {
            ...
            try
            {
                ...
                var instance = instances.OrderByDescending(inst => inst.Version).FirstOrDefault();
                ...
                _logger?.LogInformation("Registering MSBuild instance {Version} located at {Path}", instance.Version,
